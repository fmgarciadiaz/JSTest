  ...
  methods: {
    downloadData: function() {
      if (this.seriesArrayMeta.length === 0) {
        ...
        axios
          .get('http://apis.datos.gob.ar/series/api/series/', {
            params: {
              ids: idString,
              // collapse: 'quarter',
              format: 'json',
    ...
    addSeriesMeta: function(newValue) {
      if (_.find(this.seriesArrayMeta, { serie_id: newValue.serie_id }) === undefined) {
        // check if this series already exists. If it does, don't add it
    ...
    applySeriesTransformations: function() {
      let self = this
      _.map(self.seriesArrayMeta, 'internal_series_id').forEach(
        self.applySeriesTransformationsSingleSeries
      )
    ...
    applySeriesTransformationsSingleSeries: function(seriesId) {
      ...
      if (thisRepresentationMode.type === 'index_100') {
        multiplyByValueIndex100 = self.getIndex100Factor(
          seriesId,
          thisRepresentationMode.index100Date
        )
      }
    ...
    updateMetaArrayWithTransformations: function() {
      this.seriesArrayMeta = this.seriesArrayMeta.map(function(thisSeriesMeta, index) {
        ...
        // First apply representation mode transformations
        if (thisRepresentationMode.type === 'index_100') {
          newSeriesMeta.serie_descripcion_transformada =
            newSeriesMeta.serie_descripcion +
            ' (Re-indexado: ' +
            thisRepresentationMode.index100Date +
            '=100)'
          newSeriesMeta.serie_unidades_transformada =
            newSeriesMeta.serie_unidades + ' (' + thisRepresentationMode.index100Date + '=100)'
        } else if (thisRepresentationMode.type === 'change') {
          newSeriesMeta.serie_descripcion_transformada =
            newSeriesMeta.serie_descripcion + ' (Cambio)'
        } else if (thisRepresentationMode.type === 'percent_change') {
          newSeriesMeta.serie_descripcion_transformada =
            newSeriesMeta.serie_descripcion + ' (% Cambio)'
        } else if (thisRepresentationMode.type === 'percent_change_a_year_ago') {
          newSeriesMeta.serie_descripcion_transformada =
            newSeriesMeta.serie_descripcion + ' (% Cambio vs 1 año atrás)'
        } else if (thisRepresentationMode.type === 'value_with_deflactor') {
          newSeriesMeta.serie_descripcion_transformada =
            newSeriesMeta.serie_descripcion + ' (Deflactado Dic-2016=1)'
        } else {
        ...
        if (thisDataTransform.type === 'log') {
          newSeriesMeta.serie_descripcion_transformada =
            newSeriesMeta.serie_descripcion_transformada + ' (Log)'
          newSeriesMeta.serie_unidades_transformada =
            'Log (' + newSeriesMeta.serie_unidades_transformada + ')'
        } else if (thisDataTransform.type === 'multiply_by') {
          newSeriesMeta.serie_descripcion_transformada =
            newSeriesMeta.serie_descripcion_transformada +
            ' ( *' +
            Math.round(thisDataTransform.multiplyByValue * 10000) / 10000 +
            ')'
        } else {
    ...
    getPriceDeflactorSeries: function() {
      let self = this
      axios
        .get(globalStore.metadataApiBaseUrl + 'deflactor/mensual')
        .then(function(response) {
          let data = response.data.data
          self.deflactorIpc = {
            data: data,
            dates_in_ms: _.map(data, thisDayData => {
              return new Date(thisDayData[0]).getTime()
            })
          }
        })
        .catch(function(error) {
    ...
    getIndex100Factor: function(seriesId, targetDateString) {
      ...
      const theData = _.find(this.rawData.original_data, x => {
        return x[0] === targetDateString
      })
